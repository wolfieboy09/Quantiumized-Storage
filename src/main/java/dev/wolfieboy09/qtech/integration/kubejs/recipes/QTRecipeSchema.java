...
public final class QTRecipeSchema {
    ...
    private static final RecipeKey<Integer> ENERGY = NumberComponent.INT.key("energy", ComponentRole.INPUT);
    private static final RecipeKey<TickDuration> TICKS = TimeComponent.TICKS.key("ticks", ComponentRole.INPUT);
    ...
    private static RecipeSchema create(String id, QTRecipeFactory factory, RecipeKey<?>... keys) {
        return new RecipeSchema(keys).factory(new KubeRecipeFactory(ResourceHelper.asResource("qtech:" + id), TypeInfo.of(DiskAssemblerRecipe.class), () -> factory));
    }
