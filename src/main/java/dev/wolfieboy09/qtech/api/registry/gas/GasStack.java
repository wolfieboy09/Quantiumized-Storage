...
public class GasStack implements MutableDataComponentHolder {
    ...
    public static final Codec<Holder<Gas>> GAS_NON_EMPTY_CODEC = QTRegistries.GAS.holderByNameCodec().validate(holder -> holder.is(QTGasses.EMPTY.get().builtInRegistryHolder()) ? DataResult.error(() -> "Gas must not be qtech:empty") : DataResult.success(holder));
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, GasStack> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.holderRegistry(QTRegistries.GAS_KEY), GasStack::getGasHolder,
    ...
    public Gas getGas() {
        return this.isEmpty() ? QTGasses.EMPTY.get() : this.gas;
    }
