...
public class StorageMatrixMenu extends AbstractEnergyContainerMenu {
    ...
    public StorageMatrixMenu(int id, BlockPos pos, Inventory playerInventory, @NotNull Player player, ContainerData containerData) {
        super(QTMenuTypes.STORAGE_MATRIX.get(), id);
        ...
        itemStackHandler.getStackInSlot(0).set(QTDataComponents.ITEM_STORAGE_DISK_COMPONENT, new ItemStorageDiskComponent(new BaseStorageDisk(StorageType.ITEM), ItemStorageType.BASIC, NonNullList.of(new ItemStack(Items.DIAMOND, 2))));
    ...
    public boolean stillValid(@NotNull Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, QTBlocks.STORAGE_MATRIX.get());
    }
}
