...
public class PipeConnection {
    ...
    /**
     * Legacy support: Checks if a side is disconnected.
    ...
    /**
     * Legacy support: Gets all disconnected sides.
     * @return A map of directions to boolean values
     */
    public EnumMap<Direction, Boolean> getDisconnectedSides() {
        EnumMap<Direction, Boolean> result = new EnumMap<>(Direction.class);
        for (Direction dir : Direction.values()) {
            result.put(dir, getConnectionState(dir).isDisabled());
        }
        return result;
    }

    /**
     * Legacy support: Disconnects a side.
     * @param direction The direction to disconnect
     */
    public void disconnectSide(Direction direction) {
        disableConnection(direction);
    }

    /**
     * Legacy support: Reconnects a side.
     * @param direction The direction to reconnect
     */
    public void reconnectSide(Direction direction) {
        enableConnection(direction);
    }

    /**
     * Legacy support: Gets the position of this pipe.
     * @return The position
     */
    ...
    /**
     * Legacy support: Sets a connection type.
     * @param dir The direction
     * @param type The connection type
     */
    public void setConnection(Direction dir, ConnectionType type) {
        switch (type) {
            case PIPE -> setConnectionState(dir, ConnectionState.CONNECTED_TO_PIPE);
            case BLOCK -> setConnectionState(dir, ConnectionState.CONNECTED_TO_BLOCK);
            case NONE -> setConnectionState(dir, ConnectionState.NONE);
        }
    }

    /**
     * Legacy support: Gets a connection type.
     * @param dir The direction
     * @return The connection type
     */
    public ConnectionType getConnection(Direction dir) {
        return toConnectionType(getConnectionState(dir));
    }

    /**
     * Legacy support: Gets all connections.
     * @return A map of directions to connection types
     */
    public Map<Direction, ConnectionType> getAllConnections() {
        Map<Direction, ConnectionType> result = new EnumMap<>(Direction.class);
        for (Map.Entry<Direction, ConnectionState> entry : this.connectionStates.entrySet()) {
            result.put(entry.getKey(), toConnectionType(entry.getValue()));
        }
