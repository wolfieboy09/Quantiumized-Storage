package dev.wolfieboy09.qstorage.api.block;
...
public interface SimpleLavaloggedBlock extends BucketPickup, LiquidBlockContainer {
    ...
    default boolean placeLiquid(LevelAccessor level, BlockPos pos, BlockState state, FluidState fluidState) {
        if (state.getValue(QSBlockStateProperties.LAVALOGGED) && fluidState.getType() == Fluids.LAVA) {
            if (!level.isClientSide()) {
                level.setBlock(pos, state.setValue(QSBlockStateProperties.LAVALOGGED, true), 3);
    ...
    default ItemStack pickupBlock(@Nullable Player player, LevelAccessor level, BlockPos pos, BlockState state) {
        if (state.getValue(QSBlockStateProperties.LAVALOGGED)) {
            level.setBlock(pos, state.setValue(QSBlockStateProperties.LAVALOGGED, false), 3);
